    ...
    fn into_env_filter(self) -> tracing_subscriber::EnvFilter {
        match self {
            LogLevel::Debug => tracing_subscriber::EnvFilter::from_str(
                "rave_app_backend=debug,rave_embedded_database=debug,rave_api=trace,rave_entity=trace,axum=trace,axum::rejection=trace,hyper=error,axum_jwks=trace",
            ).unwrap(),
            LogLevel::Normal => tracing_subscriber::EnvFilter::from_str(
                "rave_app_backend=info,rave_embedded_database=info,rave_api=debug,rave_entity=debug,axum=info,axum::rejection=error,hyper=error,axum_jwks=warn",
            )
            .unwrap(),
